...
describe('compilteToTemplate: alipay', () => {
  ...
  it('generate v-for', () => {
    ...
    assertCodegen(
      ...
      (
        `<view a:for="{{ h[ 1 ].li }}" key="{{item.id}}" a:for-item="item" a:for-index="index" class="_div">` +
          `<view class="_div">{{ h[ 3 + '-' + (item_i2 !== undefined ? item_i2 : index) ].t }}</view>` +
        `</view>`
      )
    )
    assertCodegen(
      ...
      (
        `<view a:for="{{ h[ 1 ].li }}" key="{{item.id}}" a:for-item="item" a:for-index="index" class="_div">` +
          `<view a:for="{{ h[ 2 + '-' + (item_i2 !== undefined ? item_i2 : index) ].li }}" key="{{e.id}}" a:for-item="e" a:for-index="i" class="_div">` +
    ...
    assertCodegen(
      ...
      (
        `<view a:for="{{ h[ 1 ].li }}" key="{{item.id}}" a:for-item="item" a:for-index="item_i1" class="_div">` +
          `<view a:for="{{ h[ 2 + '-' + (item_i2 !== undefined ? item_i2 : item_i1) ].li }}" key="{{e.id}}" a:for-item="e" a:for-index="e_i1" class="_div">` +
    ...
    assertCodegen(
      ...
      (
        `<block a:for="{{ h[ 1 ].li }}" a:for-item="item" a:for-index="item_i1">` +
          `<view key="{{item.id}}" class="_div">{{ h[ 3 + '-' + (item_i2 !== undefined ? item_i2 : item_i1) ].t }}</view>` +
        `</block>`
      )
    )
  })
        ...
        res.slots.forEach((slot) => {
          if (slot.name === 'head') {
            expect(slot.body).toEqual(
              `<template name="${slot.slotName}" parent="${options.name}">` +
                `<view class="_p {{d}}">` +
                  `<view class="_span {{d}}">{{ s[ 's7' + _t ].t }}</view>` +
                `</view>` +
              `</template>`
            )
      ...
      function aasertRes (res) {
        ...
        res.slots.forEach((slot) => {
          if (slot.name === 'default') {
            expect(slot.body).toEqual(
              `<template name="${slot.slotName}" parent="${options.name}">` +
                `<view class="_span {{d}}">foo.bar</view>` +
              `</template>`
            )
          }
        })
    ...
    assertCodegen(
      ...
      function aasertRes (res) {
        res.slots.forEach((slot) => {
          ...
          expect(slot.body).toEqual(
            `<template name="${slot.slotName}" parent="${options.name}">` +
              `<view a:for="{{ s[ 's4' + (_t || '') ].li }}" a:for-item="item" a:for-index="item_i1" class="_span {{d}}">` +
    ...
    assertCodegen(
      ...
      function aasertRes (res) {
        res.slots.forEach((slot) => {
          ...
          expect(slot.body).toEqual(
            `<template name="${slot.slotName}" parent="${options.name}">` +
              `<view a:for="{{ s[ 's5' + (_t || '') ].li }}" a:for-item="item" a:for-index="item_i1" class="_span {{d}}">` +
    ...
    assertCodegen(
      ...
      function aasertRes (res) {
        res.slots.forEach((slot) => {
          ...
          expect(slot.body).toEqual(
            `<template name="${slot.slotName}" parent="${options.name}">` +
              `<view class="_span {{d}}" data-cid="{{ _c || c }}" data-hid="{{ 's4' + _t }}" onTap="_pe">` +
                `click` +
              `</view>` +
            `</template>`
          )
  ...
  it('binding named slot in native component - span', () => {
    assertCodegen(
      ...
      (
        `<view class="_div">` +
          `<CompA class="_CompA">` +
            `<view class="_span" slot="{{ h[ 3 ].slot }}">` +
              `{{ h[ 4 ].t }}` +
            `</view>` +
          `</CompA>` +
        `</view>`
